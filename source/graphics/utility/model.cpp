...
void v_model::render(shader Shader) {
...
m_mesh m_model::processMesh(aiMesh* mesh, const aiScene* scene) {
	...
	if (mesh->mMaterialIndex >= 0) {
		...
		if (noTex) {
			...
			return m_mesh(vertices, indices, diffMaps, specMaps);
		}
	...
	return m_mesh(vertices, indices, textures);
}
...
std::vector<m_texture> m_model::loadMaterialTextures(aiMaterial* material, aiTextureType type) {
	...
	for (unsigned int i = 0; i < material->GetTextureCount(type); ++i) {
		...
		if (!helperSkipFlag) {
			...
			//not loaded yet
			m_texture tex(directory, helperString.C_Str(), type);
